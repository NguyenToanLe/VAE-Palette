...
class BaseModel():
    ...
    def train(self):
        ...
        while self.epoch <= self.opt['train']['n_epoch'] and self.iter <= self.opt['train']['n_iter']:
            ...
            if self.epoch % self.opt['train']['val_epoch'] == 0:
                ...
                if self.val_loader is None:
                    ...
                    eval_score = list(val_log.values())[0]
                    if self.opt['train']['patient'] != 0:
                        if eval_score < best_eval_score:
                            ...
                            wait += 1
                            if wait >= self.opt['train']['patient']:
                                breaked = True
                                self.logger.info(
                                    "\n------------------------------Validation End------------------------------\n\n")
                                break
        ...
        if breaked:
            ...
            print(f'Best epoch: {best_epoch}, Best evaluation score: {best_eval_score}.')
            self.logger.info(f'Early stopping at epoch {self.epoch}.')
            self.logger.info(f'Best epoch: {best_epoch}, Best evaluation score: {best_eval_score}.')
        else:
            print('Number of Epochs has reached the limit, End.')
