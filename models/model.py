...
class Palette(BaseModel):
    ...
    def get_current_visuals(self, phase='train'):
        ...
        if phase != 'train':
            dict.update({
                'output': (grayscale_transform(self.output.detach()[:]).float().cpu()+1)/2
            })
        return dict
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            ...
            ret_result.append(grayscale_transform(self.visuals[idx-self.batch_size]).detach().float().cpu())
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            ...
            path_Out, path_GT = self.writer.save_images(self.save_current_results(), split_folder=True)
            for met in self.metrics:
                key = met.__name__
                _, eval_score = met(path_Out, path_GT)
                self.val_metrics.update(key, eval_score)
                self.writer.add_scalar(key, eval_score)
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                for met in self.metrics:
                    key = met.__name__
                    _, eval_score = met(path_Out, path_GT)
                    self.test_metrics.update(key, eval_score)
                    self.writer.add_scalar(key, eval_score)
