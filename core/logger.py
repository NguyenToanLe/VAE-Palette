...
class VisualWriter():
    ...
    def save_images(self, results, split_folder=False):
        ...
        try:
            ...
            if not split_folder:
                ...
                check_GT = 'GT' in ''.join(names)
                check_Mask = 'Mask' in ''.join(names)
                check_Out = 'Out' in ''.join(names)
                check_Process = 'Process' in ''.join(names)
                result_path_GT = result_path
                result_path_Mask = result_path
                result_path_Out = result_path
                result_path_Process = result_path

                if check_GT:
                    result_path_GT = os.path.join(result_path, 'GT')
                    os.makedirs(result_path_GT, exist_ok=True)
                if check_Mask:
                    result_path_Mask = os.path.join(result_path, 'Mask')
                    os.makedirs(result_path_Mask, exist_ok=True)
                if check_Out:
                    result_path_Out = os.path.join(result_path, 'Out')
                    os.makedirs(result_path_Out, exist_ok=True)
                if check_Process:
                    result_path_Process = os.path.join(result_path, 'Process')
                    os.makedirs(result_path_Process, exist_ok=True)

                for i in range(len(names)):
                    if 'GT' in names[i]:
                        Image.fromarray(outputs[i]).save(os.path.join(result_path_GT, names[i].replace("GT_", "")))
                    if 'Mask' in names[i]:
                        Image.fromarray(outputs[i]).save(os.path.join(result_path_Mask, names[i].replace("Mask_", "")))
                    if 'Out' in names[i]:
                        Image.fromarray(outputs[i]).save(os.path.join(result_path_Out, names[i].replace("Out_", "")))
                    if 'Process' in names[i]:
                        Image.fromarray(outputs[i]).save(os.path.join(result_path_Process, names[i].replace("Process_", "")))

                return result_path_Out, result_path_GT
        except:
